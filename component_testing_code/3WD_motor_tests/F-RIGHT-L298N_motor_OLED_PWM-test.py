#!/usr/bin/python
# 3WD Front right motor testing
# F-RIGHT motor uses the middle L298N controller - pins: enB, IN3, IN4
# omniwheels PCB version of L298N single motor test F-RIGHT-L298N_motor_OLED_PWM-test.py
# using a L298N to control the motor with PWM

# command to run:  python3 /home/pi3wdrobot/motor_tests_3WD_versions/F-RIGHT-L298N_motor_OLED_PWM-test.py
...
########################################################################################
def Mb_motor_pwm(Min3, Min4, dutycycle):  # allows the middle (M) L298N 'b' motor e.g. for omni wheels to be controlled
    # assumes pwm_MenB has been set up in the main code
    dutycycle = int(dutycycle) # force as an integer just in case 
    Min3 = int(Min3)           # force as an integer just in case
    Min4 = int(Min4)           # force as an integer just in case
    ...
    if dutycycle == 0:   # stop the motor using the 'hard' brake method
        # motor braking
        # set en with 100% PWM dutycycle
        pwm_MenB.start(100)
        # set Min3 off and Min4 off i.e. LOW- LOW for no motion
        GPIO.output(Min3, 0)
        GPIO.output(Min4, 0)
    elif dutycycle > 0:  # move forwards
        # set en with the PWM dutycycle
        pwm_MenB.start(dutycycle)
        # set Min3 off and Min4 on i.e. LOW - HIGH for forward motion
        GPIO.output(Min3, 0)
        GPIO.output(Min4, 1)
    elif dutycycle < 0:  # move backwards
        # set en with the PWM dutycycle
        pwm_MenB.start(abs(dutycycle))
        # set Min3 on and Min4 off i.e. HIGH - LOW for backward motion
        GPIO.output(Min3, 1)
        GPIO.output(Min4, 0)
...
# L298N setup code
# BR motor uses the middle L298N controller - pins: enB, IN3, IN4
# the middle L298N uses the 'more inboard' 6P connector on the PCB
MenB = 10   # this will be a software set PWM pin
Min3 = 11
Min4 = 9

# Set the GPIO Pin mode
GPIO.setup(MenB, GPIO.OUT)
...
pwm_MenB = GPIO.PWM(MenB, Frequency)  # set the enA pin as a software set PWM pin
# Start the software PWM pins with a duty cycle of 0 (i.e. motors not moving)
pwm_MenB.start(0)

# Turn motor off - hard brake
Mb_motor_pwm(Min3, Min4, 0)
line1 = "Driving motor:"
line2 = "motor off"
line3 = " "
line4 = " "
